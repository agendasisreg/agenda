...
function initAutocompletes() {
    ...
    setupAutocomplete(els.inputProcedimento, els.listProcedimentos, DB.procedimentos, 'nome', 'codigo', (item) => {
        ...
        // REGRA: Se o procedimento for FINANCEIRO, a escala deve ser sempre "ORDEM DE CHEGADA" (0)
        if (item.tipo && item.tipo.includes('FINANCEIRO')) {
            els.tipoEscala.value = "0";
            els.tipoEscala.disabled = true;
            // Força também a atualização do campo minutos para 1 conforme a lógica da escala 0
    ...
    // NOVA REGRA: Bloqueio de intervalos de 5 minutos na Hora Inicial
    els.horaInicio.setAttribute('step', '300'); // Define step de 5 min para seletores nativos
...
els.btnExportar.onclick = () => {
    ...
    AppState.escalas.forEach(l => {
        ...
        } else if (l.tp_agenda === "1" || l.isRetorno) {
            // Se NÃO for regulado, mas tp_agenda for Local (1) OU for Retorno, vai para a coluna retornos
            retornos = vCsv;
            retornosMin = l.minutos;
        } else {
            // Se não for nenhum dos casos acima, vai para fichas
            fichas = vCsv;
            fichasMin = l.minutos;
        }
